...
def index():
    ...
    rows = ''.join(f"<tr><td>{r[1]}</td><td>{r[2]}</td><td><a href='/edit/{r[0]}'>Edit</a> <a href='/delete/{r[0]}'>Delete</a></td></tr>" for r in cur.fetchall()) or "<tr><td colspan=3>No messages</td></tr>"
    conn.close()
    return HTML % rows
...
def create():
    conn = db()
    cur = conn.cursor()
    cur.execute("INSERT INTO messages (name, message) VALUES (%s, %s)", (request.form['name'], request.form['message']))
    conn.commit()
    conn.close()
    return redirect('/')
...
def edit(id):
    ...
    if request.method == 'POST':
        cur.execute("UPDATE messages SET name=%s, message=%s WHERE id=%s", (request.form['name'], request.form['message'], id))
        conn.commit()
        conn.close()
        return redirect('/')
...
<input name=name value="{r[0]}" required>
<textarea name=message required>{r[1]}</textarea>
